...
// Yank: enable/disable sysfs interface to display current interactive version
#define INTERACTIVE_VERSION "1.0 beta8"
...
#include "cpufreq_interactive_profiles.h"
...
static struct notifier_block interactive_lcd_notif;
	...
	if (type == EV_SYN && code == SYN_REPORT) {
#ifdef INTERACTIVE_DEBUG
		pr_info("[interactive] syn input event. device: %s, saw_touchmajor: %d, saw_xy: %d, toolfinger: %d, touch: %d, trackingid: %d\n",
		...
		if (strstr(handle->dev->name, "touchscreen") || strstr(handle->dev->name, "synaptics")) {
			...
			if (!flg_inputboost_mt_touchmajor
				...
				if (inputboost_mt_trackingid > 0) {
					...
					pr_info("[interactive] touch - first hover btn_touch: %d\n", inputboost_btn_touch);
					/*// ff: unused, but kept for future use
					flg_do_punch_id = 4;
					flg_force_punch = false;*/
				} else if (inputboost_mt_trackingid < 0) {

					pr_info("[interactive] touch - end hover btn_touch: %d\n", inputboost_btn_touch);
					/*// ff: don't boost if not enabled, or while sleeping
					flg_ctr_inputboost_punch = 0;
					flg_ctr_inputboost = 0;*/
				} else {
					pr_info("[interactive] touch - update hover btn_touch: %d\n", inputboost_btn_touch);
				}
#endif /* INTERACTIVE_DEBUG */
			} else if (inputboost_mt_trackingid > 0) {
				// ff: new finger detected event
#ifdef INTERACTIVE_DEBUG
				pr_info("[interactive] touch - first touch\n");
#endif /* INTERACTIVE_DEBUG */
				flg_do_punch_id = 2;
				...
				if (dbs_tuners_ins.inputboost_typingbooster_up_threshold) {
					...
					if ((flg_ctr_inputbooster_typingbooster < 1 && ctr_inputboost_typingbooster_taps > 1)			// ff: if booster wasn't on, require 3 taps
						...
						if (flg_ctr_inputbooster_typingbooster < 1)
						    pr_info("[interactive] inputboost - typing booster on!\n");
#endif /* INTERACTIVE_DEBUG */
...
#ifdef INTERACTIVE_DEBUG
				pr_info("[interactive] touch - unknown\n");
#endif /* INTERACTIVE_DEBUG */
				return;
			}
			...
			if (inputboost_last_code == 172) {
				if (suspend_flag) {
					...
					flg_ctr_cpuboost = 2;
#ifdef INTERACTIVE_DEBUG
					pr_info("[interactive] inputboost - gpio punched freq immediately!\n");
#endif /* INTERACTIVE_DEBUG */
				} else {
...
#ifdef ENABLE_WORK_RESTARTLOOP
#ifdef INTERACTIVE_DEBUG
			pr_info("[interactive] inputboost - tk punched freq immediately!\n");
#endif /* INTERACTIVE_DEBUG */
			queue_work_on(0, dbs_aux_wq, &work_restartloop);
#endif /* ENABLE_WORK_RESTARTLOOP */
			// ff: don't punch, since we just did manually
			...
			if (inputboost_last_code == 116 && suspend_flag) {
				...
				queue_work_on(0, dbs_aux_wq, &work_restartloop);
				pr_info("[interactive] inputboost - gpio/powerkey punched freq immediately and skipped the rest\n");*/

				// ff: not only don't punch, but don't even start the booster, since we just did both with interactive_boost() externally
				return;
			} else {
		...
		if (flg_do_punch_id						// ff: punch is requested
			...
			if (flg_ctr_cpuboost < 5 && tmp_flg_ctr_inputboost_punch < 1) {
				...
				pr_info("[interactive] inputboost - punched freq immediately for: %d\n", flg_do_punch_id);
#endif /* INTERACTIVE_DEBUG */
#ifdef ENABLE_WORK_RESTARTLOOP
			}
#endif /* ENABLE_WORK_RESTARTLOOP */
#ifdef INTERACTIVE_DEBUG
			pr_info("[interactive] inputboost - punch set to %d mhz for %d cycles (punched by: %d, forced: %d)\n",
...
#ifdef INTERACTIVE_DEBUG
		pr_info("[interactive] ev input event. name: %s, type: %d, code: %d, value: %d\n", handle->dev->name, type, code, value);
#endif /* INTERACTIVE_DEBUG */
...
#ifdef INTERACTIVE_DEBUG
		pr_info("[interactive] inputboost - monitoring input device: %s\n", input_dev_name);
#endif /* INTERACTIVE_DEBUG */
		return 0;

	} else {
#ifdef INTERACTIVE_DEBUG
		pr_info("[interactive] inputboost - ignored input device: %s\n", input_dev_name);
#endif /* INTERACTIVE_DEBUG */
		return 1;
	}
}
...
static void interactive_input_disconnect(struct input_handle *handle)
{
	...
	flg_ctr_inputboost_punch = 0;
#ifdef INTERACTIVE_DEBUG
	pr_info("[interactive/interactive_input_disconnect] inputbooster - unregistering\n");
#endif /* INTERACTIVE_DEBUG */
...
static inline int zz_get_next_freq(unsigned int curfreq, unsigned int updown, unsigned int load)
{
	...
	if (freq_table_desc) {
...
#ifdef INTERACTIVE_DEBUG
			pr_info("[interactive/zz_get_next_freq] desc - UP - val1: %d - val2: %d [steps: %d, smooth_up: %d, scaling_mode_up: %d]\n",
...
#ifdef INTERACTIVE_DEBUG
			pr_info("[interactive/zz_get_next_freq] desc - DOWN - val1: %d - val2: %d [steps: %d, scaling_mode_down: %d]\n",
		...
		if (unlikely(curfreq == system_freq_table[i].frequency)) {	// Yank: we found where we currently are (i)
			...
			system_freq_table[validate_min_max(i + 1 + smooth_up_steps + scaling_mode_up, 0, freq_table_size)].frequency);
#ifdef INTERACTIVE_DEBUG
			pr_info("[interactive/zz_get_next_freq] asc - UP - val1: %d - val2: %d [steps: %d, smooth_up: %d, scaling_mode_up: %d]\n",
				system_freq_table[tmp_max_scaling_freq_soft].frequency, system_freq_table[validate_min_max(i + 1 + smooth_up_steps + scaling_mode_up,
				0, freq_table_size)].frequency, (1 + smooth_up_steps + scaling_mode_up), smooth_up_steps, scaling_mode_up);
#endif /* INTERACTIVE_DEBUG */
			if (dbs_tuners_ins.scaling_proportional == 1)		// ZZ: if proportional scaling is enabled
			...
			system_freq_table[validate_min_max(i - 1 - scaling_mode_down, 0, freq_table_size)].frequency);
#ifdef INTERACTIVE_DEBUG
			pr_info("[interactive/zz_get_next_freq] asc - DOWN - val1: %d - val2: %d [steps: %d, scaling_mode_down: %d]\n",
	...
	if (enable_cores == 2) {							// ZZ: limit min cores
		for (i = 1; i < num_possible_cpus(); i++) {
		    ...
		    && (!dbs_tuners_ins.hotplug_max_limit || i < dbs_tuners_ins.hotplug_max_limit)) {
			    ...
			    cpu_up(i);
#ifdef INTERACTIVE_DEBUG
			    pr_info("[interactive] hotplug_min_limit: CPU%d forced up\n", i);
#endif /* INTERACTIVE_DEBUG */
		    }
		}
	...
	if (disable_cores == 1) {							// ZZ: limit max cores
		for (i = num_possible_cpus() - 1; i >= 1; i--) {
		    ...
		    || i >= dbs_tuners_ins.hotplug_min_limit)) {
			    ...
			    cpu_down(i);
#ifdef INTERACTIVE_DEBUG
			    pr_info("[interactive] hotplug_max_limit: CPU%d forced down\n", i);
#endif /* INTERACTIVE_DEBUG */
		    }
		}
...
static inline unsigned int check_frequency(unsigned int input_freq)
{
	...
	if (found_freq != 0) {
#ifdef INTERACTIVE_DEBUG
	    pr_info("[interactive] check_freq: freq %d diretly found!\n", found_freq);
#endif /* INTERACTIVE_DEBUG */
	    return found_freq;
	}
...
#ifdef INTERACTIVE_DEBUG
	    pr_info("[interactive] check_freq: minimal possible freq %d\n", system_min_freq);
#endif /* INTERACTIVE_DEBUG */
		    break;
		}
	    }
...
#ifdef INTERACTIVE_DEBUG
	    pr_info("[interactive] check_freq: maximal possible freq %d\n", system_max_freq);
#endif /* INTERACTIVE_DEBUG */
		    break;
		}
	    }
...
#ifdef INTERACTIVE_DEBUG
	    pr_info("[interactive] check_freq: input freq too high switching to %d\n", system_max_freq);
#endif /* INTERACTIVE_DEBUG */
	    return system_max_freq;
	} else if (input_freq < system_min_freq) {
#ifdef INTERACTIVE_DEBUG
	    pr_info("[interactive] check_freq: input freq too low switching to %d\n", system_min_freq);
#endif /* INTERACTIVE_DEBUG */
...
#ifdef INTERACTIVE_DEBUG
	pr_info("[interactive] check_freq: input freq was %d\n", input_freq);
	pr_info("[interactive] check_freq: calculated freq is %d\n", calc_freq);
	pr_info("[interactive] check_freq: relation to profile in per mille %d\n", profile_relation);
	pr_info("[interactive] check_freq: lower freq %d on index %d", system_freq_table[i].frequency, i);
	pr_info("[interactive] check_freq: higher freq %d on index %d", system_freq_table[next_freq].frequency, next_freq);
	pr_info("[interactive] check_freq: found freq %d / mid freq %d / adapted freq %d\n", found_freq, mid_freq, adapted_freq);
#endif /* INTERACTIVE_DEBUG */
...
// cpufreq_interactive governor tunables
									   ...
									   const char *buf, size_t count)
{
	...
	if (!input && dbs_tuners_ins.inputboost_cycles != input) {
		...
		// so add booster and register.
		    ...
		    if (!rc)
			    pr_info("[interactive/store_inputboost_cycles] inputbooster - registered\n");
		    else
			    pr_info("[interactive/store_inputboost_cycles] inputbooster - register FAILED\n");
	}
...
static inline int set_profile(int profile_num)
{
	...
	for (i = 0; (unlikely(interactive_profiles[i].profile_number != PROFILE_TABLE_END)); i++) {
	    if (unlikely(interactive_profiles[i].profile_number == profile_num)) {

#ifdef ENABLE_HOTPLUGGING
		// ZZ: set disable_hotplug value
		if (interactive_profiles[i].disable_hotplug > 0 && interactive_profiles[i].disable_hotplug < 2) {
		    dbs_tuners_ins.disable_hotplug = interactive_profiles[i].disable_hotplug;

		    if (interactive_profiles[i].disable_hotplug == 1) {
			enable_cores = 1;
			queue_work_on(0, dbs_wq, &hotplug_online_work);
		    }
		}
...
#if (defined(CONFIG_HAS_EARLYSUSPEND) || defined(CONFIG_POWERSUSPEND) && !defined(DISABLE_POWER_MANAGEMENT)) || defined(USE_LCD_NOTIFIER)
		if (interactive_profiles[i].disable_hotplug_sleep > 0 && interactive_profiles[i].disable_hotplug_sleep < 2)
		    dbs_tuners_ins.disable_hotplug_sleep = interactive_profiles[i].disable_hotplug_sleep;

		// ZZ: set hotplug_sleep value
		if (interactive_profiles[i].hotplug_sleep <= possible_cpus && interactive_profiles[i].hotplug_sleep >= 0)
		    dbs_tuners_ins.hotplug_sleep = interactive_profiles[i].hotplug_sleep;
#endif /* (defined(CONFIG_HAS_EARLYSUSPEND)... */
#endif /* ENABLE_HOTPLUGGING */
		// ZZ: set down_threshold value
		if (interactive_profiles[i].down_threshold > 11 && interactive_profiles[i].down_threshold <= 100
		    && interactive_profiles[i].down_threshold < interactive_profiles[i].up_threshold)
		    dbs_tuners_ins.down_threshold = interactive_profiles[i].down_threshold;

#ifdef ENABLE_HOTPLUGGING
		// ZZ: set down_threshold_hotplug1 value
		if ((interactive_profiles[i].down_threshold_hotplug1 <= 100
		    && interactive_profiles[i].down_threshold_hotplug1 >= 1)
		    || interactive_profiles[i].down_threshold_hotplug1 == 0) {
		    dbs_tuners_ins.down_threshold_hotplug1 = interactive_profiles[i].down_threshold_hotplug1;
		    hotplug_thresholds[1][0] = interactive_profiles[i].down_threshold_hotplug1;
		}

		// ff: set block up multiplier for hotplug1
		if (interactive_profiles[i].block_up_multiplier_hotplug1 >= 0 && interactive_profiles[i].block_up_multiplier_hotplug1 <= 25)
		    dbs_tuners_ins.block_up_multiplier_hotplug1 = interactive_profiles[i].block_up_multiplier_hotplug1;

		// ff: set block down multiplier for hotplug1
		if (interactive_profiles[i].block_down_multiplier_hotplug1 >= 0 && interactive_profiles[i].block_down_multiplier_hotplug1 <= 25)
		    dbs_tuners_ins.block_down_multiplier_hotplug1 = interactive_profiles[i].block_down_multiplier_hotplug1;
#if (MAX_CORES == 4 || MAX_CORES == 6 || MAX_CORES == 8)
		// ZZ: set down_threshold_hotplug2 value
		if ((interactive_profiles[i].down_threshold_hotplug2 <= 100
		    && interactive_profiles[i].down_threshold_hotplug2 >= 1)
		    || interactive_profiles[i].down_threshold_hotplug2 == 0) {
		    dbs_tuners_ins.down_threshold_hotplug2 = interactive_profiles[i].down_threshold_hotplug2;
		    hotplug_thresholds[1][1] = interactive_profiles[i].down_threshold_hotplug2;
		}

		// ff: set block up multiplier for hotplug2
		if (interactive_profiles[i].block_up_multiplier_hotplug2 >= 0 && interactive_profiles[i].block_up_multiplier_hotplug2 <= 25)
		    dbs_tuners_ins.block_up_multiplier_hotplug2 = interactive_profiles[i].block_up_multiplier_hotplug2;

		// ff: set block down multiplier for hotplug2
		if (interactive_profiles[i].block_down_multiplier_hotplug2 >= 0 && interactive_profiles[i].block_down_multiplier_hotplug2 <= 25)
		    dbs_tuners_ins.block_down_multiplier_hotplug2 = interactive_profiles[i].block_down_multiplier_hotplug2;

		// ZZ: set down_threshold_hotplug3 value
		if ((interactive_profiles[i].down_threshold_hotplug3 <= 100
		    && interactive_profiles[i].down_threshold_hotplug3 >= 1)
		    || interactive_profiles[i].down_threshold_hotplug3 == 0) {
		    dbs_tuners_ins.down_threshold_hotplug3 = interactive_profiles[i].down_threshold_hotplug3;
		    hotplug_thresholds[1][2] = interactive_profiles[i].down_threshold_hotplug3;
		}

		// ff: set block up multiplier for hotplug3
		if (interactive_profiles[i].block_up_multiplier_hotplug3 >= 0 && interactive_profiles[i].block_up_multiplier_hotplug3 <= 25)
		    dbs_tuners_ins.block_up_multiplier_hotplug3 = interactive_profiles[i].block_up_multiplier_hotplug3;

		// ff: set block down multiplier for hotplug3
		if (interactive_profiles[i].block_down_multiplier_hotplug3 >= 0 && interactive_profiles[i].block_down_multiplier_hotplug3 <= 25)
		    dbs_tuners_ins.block_down_multiplier_hotplug3 = interactive_profiles[i].block_down_multiplier_hotplug3;
#endif /* (MAX_CORES == 4 || MAX_CORES == 6 || MAX_CORES == 8) */
#if (MAX_CORES == 6 || MAX_CORES == 8)
		// ZZ: set down_threshold_hotplug4 value
		if ((interactive_profiles[i].down_threshold_hotplug4 <= 100
		    && interactive_profiles[i].down_threshold_hotplug4 >= 1)
		    || interactive_profiles[i].down_threshold_hotplug4 == 0) {
		    dbs_tuners_ins.down_threshold_hotplug4 = interactive_profiles[i].down_threshold_hotplug4;
		    hotplug_thresholds[1][3] = interactive_profiles[i].down_threshold_hotplug4;
		}

		// ff: set block up multiplier for hotplug4
		if (interactive_profiles[i].block_up_multiplier_hotplug4 >= 0 && interactive_profiles[i].block_up_multiplier_hotplug4 <= 25)
		    dbs_tuners_ins.block_up_multiplier_hotplug4 = interactive_profiles[i].block_up_multiplier_hotplug4;

		// ff: set block down multiplier for hotplug4
		if (interactive_profiles[i].block_down_multiplier_hotplug4 >= 0 && interactive_profiles[i].block_down_multiplier_hotplug4 <= 25)
		    dbs_tuners_ins.block_down_multiplier_hotplug4 = interactive_profiles[i].block_down_multiplier_hotplug4;

		// ZZ: set down_threshold_hotplug5 value
		if ((interactive_profiles[i].down_threshold_hotplug5 <= 100
		    && interactive_profiles[i].down_threshold_hotplug5 >= 1)
		    || interactive_profiles[i].down_threshold_hotplug5 == 0) {
		    dbs_tuners_ins.down_threshold_hotplug5 = interactive_profiles[i].down_threshold_hotplug5;
		    hotplug_thresholds[1][4] = interactive_profiles[i].down_threshold_hotplug5;
		}

		// ff: set block up multiplier for hotplug5
		if (interactive_profiles[i].block_up_multiplier_hotplug5 >= 0 && interactive_profiles[i].block_up_multiplier_hotplug5 <= 25)
		    dbs_tuners_ins.block_up_multiplier_hotplug5 = interactive_profiles[i].block_up_multiplier_hotplug5;

		// ff: set block down multiplier for hotplug5
		if (interactive_profiles[i].block_down_multiplier_hotplug5 >= 0 && interactive_profiles[i].block_down_multiplier_hotplug5 <= 25)
		    dbs_tuners_ins.block_down_multiplier_hotplug5 = interactive_profiles[i].block_down_multiplier_hotplug5;

		// ZZ: set down_threshold_hotplug6 value
		if ((interactive_profiles[i].down_threshold_hotplug6 <= 100
		    && interactive_profiles[i].down_threshold_hotplug6 >= 1)
		    || interactive_profiles[i].down_threshold_hotplug6 == 0) {
		    dbs_tuners_ins.down_threshold_hotplug6 = interactive_profiles[i].down_threshold_hotplug6;
		    hotplug_thresholds[1][5] = interactive_profiles[i].down_threshold_hotplug6;
		}
#endif /* (MAX_CORES == 6 || MAX_CORES == 8) */
#if (MAX_CORES == 8)
		// ff: set block up multiplier for hotplug6
		if (interactive_profiles[i].block_up_multiplier_hotplug6 >= 0 && interactive_profiles[i].block_up_multiplier_hotplug6 <= 25)
		    dbs_tuners_ins.block_up_multiplier_hotplug6 = interactive_profiles[i].block_up_multiplier_hotplug6;

		// ff: set block down multiplier for hotplug6
		if (interactive_profiles[i].block_down_multiplier_hotplug6 >= 0 && interactive_profiles[i].block_down_multiplier_hotplug6 <= 25)
		    dbs_tuners_ins.block_down_multiplier_hotplug6 = interactive_profiles[i].block_down_multiplier_hotplug6;

		// ZZ: set down_threshold_hotplug7 value
		if ((interactive_profiles[i].down_threshold_hotplug7 <= 100
		    && interactive_profiles[i].down_threshold_hotplug7 >= 1)
		    || interactive_profiles[i].down_threshold_hotplug7 == 0) {
		    dbs_tuners_ins.down_threshold_hotplug7 = interactive_profiles[i].down_threshold_hotplug7;
		    hotplug_thresholds[1][6] = interactive_profiles[i].down_threshold_hotplug7;
		}

		// ff: set block up multiplier for hotplug7
		if (interactive_profiles[i].block_up_multiplier_hotplug7 >= 0 && interactive_profiles[i].block_up_multiplier_hotplug7 <= 25)
		    dbs_tuners_ins.block_up_multiplier_hotplug7 = interactive_profiles[i].block_up_multiplier_hotplug7;

		// ff: set block down multiplier for hotplug7
		if (interactive_profiles[i].block_down_multiplier_hotplug7 >= 0 && interactive_profiles[i].block_down_multiplier_hotplug7 <= 25)
		    dbs_tuners_ins.block_down_multiplier_hotplug7 = interactive_profiles[i].block_down_multiplier_hotplug7;

#endif /* (MAX_CORES == 8) */
		// ZZ: set down_threshold_hotplug_freq1 value
		if (interactive_profiles[i].down_threshold_hotplug_freq1 == 0) {
		    dbs_tuners_ins.down_threshold_hotplug_freq1 = interactive_profiles[i].down_threshold_hotplug_freq1;
		    hotplug_thresholds_freq[1][0] = interactive_profiles[i].down_threshold_hotplug_freq1;
		}

		interactive_profiles[i].down_threshold_hotplug_freq1 = check_frequency(interactive_profiles[i].down_threshold_hotplug_freq1);
		if (system_freq_table && interactive_profiles[i].down_threshold_hotplug_freq1 <= system_freq_table[max_scaling_freq_hard].frequency) {
		    for (t = 0; (system_freq_table[t].frequency != system_table_end); t++) {
			if (system_freq_table[t].frequency == interactive_profiles[i].down_threshold_hotplug_freq1) {
			    dbs_tuners_ins.down_threshold_hotplug_freq1 = interactive_profiles[i].down_threshold_hotplug_freq1;
			    hotplug_thresholds_freq[1][0] = interactive_profiles[i].down_threshold_hotplug_freq1;
			}
		    }
		...
		if (interactive_profiles[i].down_threshold_hotplug_freq2 == 0) {
		    dbs_tuners_ins.down_threshold_hotplug_freq2 = interactive_profiles[i].down_threshold_hotplug_freq2;
		    hotplug_thresholds_freq[1][1] = interactive_profiles[i].down_threshold_hotplug_freq2;
		}

		interactive_profiles[i].down_threshold_hotplug_freq2 = check_frequency(interactive_profiles[i].down_threshold_hotplug_freq2);
		if (system_freq_table && interactive_profiles[i].down_threshold_hotplug_freq2 <= system_freq_table[max_scaling_freq_hard].frequency) {
		    for (t = 0; (system_freq_table[t].frequency != system_table_end); t++) {
			if (system_freq_table[t].frequency == interactive_profiles[i].down_threshold_hotplug_freq2) {
			    dbs_tuners_ins.down_threshold_hotplug_freq2 = interactive_profiles[i].down_threshold_hotplug_freq2;
			    hotplug_thresholds_freq[1][1] = interactive_profiles[i].down_threshold_hotplug_freq2;
			}
		    }
		...
		if (interactive_profiles[i].down_threshold_hotplug_freq3 == 0) {
		    dbs_tuners_ins.down_threshold_hotplug_freq3 = interactive_profiles[i].down_threshold_hotplug_freq3;
		    hotplug_thresholds_freq[1][2] = interactive_profiles[i].down_threshold_hotplug_freq3;
		}

		interactive_profiles[i].down_threshold_hotplug_freq3 = check_frequency(interactive_profiles[i].down_threshold_hotplug_freq3);
		if (system_freq_table && interactive_profiles[i].down_threshold_hotplug_freq3 <= system_freq_table[max_scaling_freq_hard].frequency) {
		    for (t = 0; (system_freq_table[t].frequency != system_table_end); t++) {
			if (system_freq_table[t].frequency == interactive_profiles[i].down_threshold_hotplug_freq3) {
			    dbs_tuners_ins.down_threshold_hotplug_freq3 = interactive_profiles[i].down_threshold_hotplug_freq3;
			    hotplug_thresholds_freq[1][2] = interactive_profiles[i].down_threshold_hotplug_freq3;
			}
		    }
		...
		if (interactive_profiles[i].down_threshold_hotplug_freq4 == 0) {
		    dbs_tuners_ins.down_threshold_hotplug_freq4 = interactive_profiles[i].down_threshold_hotplug_freq4;
		    hotplug_thresholds_freq[1][3] = interactive_profiles[i].down_threshold_hotplug_freq4;
		}

		interactive_profiles[i].down_threshold_hotplug_freq4 = check_frequency(interactive_profiles[i].down_threshold_hotplug_freq4);
		if (system_freq_table && interactive_profiles[i].down_threshold_hotplug_freq4 <= system_freq_table[max_scaling_freq_hard].frequency) {
		    for (t = 0; (system_freq_table[t].frequency != system_table_end); t++) {
			if (system_freq_table[t].frequency == interactive_profiles[i].down_threshold_hotplug_freq4) {
			    dbs_tuners_ins.down_threshold_hotplug_freq4 = interactive_profiles[i].down_threshold_hotplug_freq4;
			    hotplug_thresholds_freq[1][3] = interactive_profiles[i].down_threshold_hotplug_freq4;
			}
		    }
		...
		if (interactive_profiles[i].down_threshold_hotplug_freq5 == 0) {
		    dbs_tuners_ins.down_threshold_hotplug_freq5 = interactive_profiles[i].down_threshold_hotplug_freq5;
		    hotplug_thresholds_freq[1][4] = interactive_profiles[i].down_threshold_hotplug_freq5;
		}

		interactive_profiles[i].down_threshold_hotplug_freq5 = check_frequency(interactive_profiles[i].down_threshold_hotplug_freq5);
		if (system_freq_table && interactive_profiles[i].down_threshold_hotplug_freq5 <= system_freq_table[max_scaling_freq_hard].frequency) {
		    for (t = 0; (system_freq_table[t].frequency != system_table_end); t++) {
			if (system_freq_table[t].frequency == interactive_profiles[i].down_threshold_hotplug_freq5) {
			    dbs_tuners_ins.down_threshold_hotplug_freq5 = interactive_profiles[i].down_threshold_hotplug_freq5;
			    hotplug_thresholds_freq[1][4] = interactive_profiles[i].down_threshold_hotplug_freq5;
			}
		    }
		...
		if (interactive_profiles[i].down_threshold_hotplug_freq6 == 0) {
		    dbs_tuners_ins.down_threshold_hotplug_freq6 = interactive_profiles[i].down_threshold_hotplug_freq6;
		    hotplug_thresholds_freq[1][5] = interactive_profiles[i].down_threshold_hotplug_freq6;
		}

		interactive_profiles[i].down_threshold_hotplug_freq6 = check_frequency(interactive_profiles[i].down_threshold_hotplug_freq6);
		if (system_freq_table && interactive_profiles[i].down_threshold_hotplug_freq6 <= system_freq_table[max_scaling_freq_hard].frequency) {
		    for (t = 0; (system_freq_table[t].frequency != system_table_end); t++) {
			if (system_freq_table[t].frequency == interactive_profiles[i].down_threshold_hotplug_freq6) {
			    dbs_tuners_ins.down_threshold_hotplug_freq6 = interactive_profiles[i].down_threshold_hotplug_freq6;
			    hotplug_thresholds_freq[1][5] = interactive_profiles[i].down_threshold_hotplug_freq6;
			}
		    }
		...
		if (interactive_profiles[i].down_threshold_hotplug_freq7 == 0) {
		    dbs_tuners_ins.down_threshold_hotplug_freq7 = interactive_profiles[i].down_threshold_hotplug_freq7;
		    hotplug_thresholds_freq[1][6] = interactive_profiles[i].down_threshold_hotplug_freq7;
		}

		interactive_profiles[i].down_threshold_hotplug_freq7 = check_frequency(interactive_profiles[i].down_threshold_hotplug_freq7);
		if (system_freq_table && interactive_profiles[i].down_threshold_hotplug_freq7 <= system_freq_table[max_scaling_freq_hard].frequency) {
		    for (t = 0; (system_freq_table[t].frequency != system_table_end); t++) {
			if (system_freq_table[t].frequency == interactive_profiles[i].down_threshold_hotplug_freq7) {
			    dbs_tuners_ins.down_threshold_hotplug_freq7 = interactive_profiles[i].down_threshold_hotplug_freq7;
			    hotplug_thresholds_freq[1][6] = interactive_profiles[i].down_threshold_hotplug_freq7;
			}
		    }
		...
		if (interactive_profiles[i].down_threshold_sleep > 11 && interactive_profiles[i].down_threshold_sleep <= 100
		    && interactive_profiles[i].down_threshold_sleep < dbs_tuners_ins.up_threshold_sleep)
		    dbs_tuners_ins.down_threshold_sleep = interactive_profiles[i].down_threshold_sleep;
#endif /* (defined(CONFIG_HAS_EARLYSUSPEND)... */
		// ZZ: set early_demand value
		dbs_tuners_ins.early_demand = !!interactive_profiles[i].early_demand;
#if (defined(CONFIG_HAS_EARLYSUSPEND) || defined(CONFIG_POWERSUSPEND) && !defined(DISABLE_POWER_MANAGEMENT)) || defined(USE_LCD_NOTIFIER)
		dbs_tuners_ins.early_demand_sleep = !!interactive_profiles[i].early_demand_sleep;
#endif /* (defined(CONFIG_HAS_EARLYSUSPEND)... */
		// Yank: set fast_scaling value
		if (interactive_profiles[i].fast_scaling_up <= 5 && interactive_profiles[i].fast_scaling_up >= 0) {
			dbs_tuners_ins.fast_scaling_up = interactive_profiles[i].fast_scaling_up;
			if (interactive_profiles[i].fast_scaling_up > 4)
				scaling_mode_up = 0;
			else
				scaling_mode_up = interactive_profiles[i].fast_scaling_up;
		}

		if (interactive_profiles[i].fast_scaling_down <= 5 && interactive_profiles[i].fast_scaling_down >= 0) {
			dbs_tuners_ins.fast_scaling_down = interactive_profiles[i].fast_scaling_down;
			if (interactive_profiles[i].fast_scaling_down > 4)
				scaling_mode_down = 0;
			else
				scaling_mode_down = interactive_profiles[i].fast_scaling_down;
		}
#if (defined(CONFIG_HAS_EARLYSUSPEND) || defined(CONFIG_POWERSUSPEND) && !defined(DISABLE_POWER_MANAGEMENT)) || defined(USE_LCD_NOTIFIER)
		// ZZ: set fast_scaling_sleep value
		if (interactive_profiles[i].fast_scaling_sleep_up <= 5 && interactive_profiles[i].fast_scaling_sleep_up >= 0)
			dbs_tuners_ins.fast_scaling_sleep_up = interactive_profiles[i].fast_scaling_sleep_up;

		if (interactive_profiles[i].fast_scaling_sleep_down <= 5 && interactive_profiles[i].fast_scaling_sleep_down >= 0)
			dbs_tuners_ins.fast_scaling_sleep_down = interactive_profiles[i].fast_scaling_sleep_down;
#endif /* (defined(CONFIG_HAS_EARLYSUSPEND)... */
		// ZZ: set afs_threshold1 value
		if (interactive_profiles[i].afs_threshold1 <= 100)
		    dbs_tuners_ins.afs_threshold1 = interactive_profiles[i].afs_threshold1;

		// ZZ: set afs_threshold2 value
		if (interactive_profiles[i].afs_threshold2 <= 100)
		    dbs_tuners_ins.afs_threshold2 = interactive_profiles[i].afs_threshold2;

		// ZZ: set afs_threshold3 value
		if (interactive_profiles[i].afs_threshold3 <= 100)
		    dbs_tuners_ins.afs_threshold3 = interactive_profiles[i].afs_threshold3;

		// ZZ: set afs_threshold4 value
		if (interactive_profiles[i].afs_threshold4 <= 100)
		    dbs_tuners_ins.afs_threshold4 = interactive_profiles[i].afs_threshold4;

		// ZZ: set freq_limit value
		if (system_freq_table && interactive_profiles[i].freq_limit == 0) {
		    max_scaling_freq_soft = max_scaling_freq_hard;
...
#if (defined(CONFIG_HAS_EARLYSUSPEND) || defined(CONFIG_POWERSUSPEND) && !defined(DISABLE_POWER_MANAGEMENT)) || defined(USE_LCD_NOTIFIER)
		    freq_limit_awake = dbs_tuners_ins.freq_limit = interactive_profiles[i].freq_limit;
#else
		    dbs_tuners_ins.freq_limit = interactive_profiles[i].freq_limit;
#endif /* (defined(CONFIG_HAS_EARLYSUSPEND)... */
		} else if (system_freq_table && interactive_profiles[i].freq_limit <= system_freq_table[max_scaling_freq_hard].frequency) {
		    interactive_profiles[i].freq_limit = check_frequency(interactive_profiles[i].freq_limit);
		    for (t = 0; (system_freq_table[t].frequency != system_table_end); t++) {
			if (system_freq_table[t].frequency == interactive_profiles[i].freq_limit) {
...
#if (defined(CONFIG_HAS_EARLYSUSPEND) || defined(CONFIG_POWERSUSPEND) && !defined(DISABLE_POWER_MANAGEMENT)) || defined(USE_LCD_NOTIFIER)
		if (system_freq_table && interactive_profiles[i].freq_limit_sleep == 0) {
		    freq_limit_asleep = dbs_tuners_ins.freq_limit_sleep = interactive_profiles[i].freq_limit_sleep;

		} else if (system_freq_table && interactive_profiles[i].freq_limit_sleep <= system_freq_table[max_scaling_freq_hard].frequency) {
		    interactive_profiles[i].freq_limit_sleep = check_frequency(interactive_profiles[i].freq_limit_sleep);
		    for (t = 0; (system_freq_table[t].frequency != system_table_end); t++) {
			if (system_freq_table[t].frequency == interactive_profiles[i].freq_limit_sleep)
			    freq_limit_asleep = dbs_tuners_ins.freq_limit_sleep = interactive_profiles[i].freq_limit_sleep;
		    }
		}
#endif /* (defined(CONFIG_HAS_EARLYSUSPEND)... */
		// ZZ: set grad_up_threshold value
		if (interactive_profiles[i].grad_up_threshold < 100 && interactive_profiles[i].grad_up_threshold > 1)
		    dbs_tuners_ins.grad_up_threshold = interactive_profiles[i].grad_up_threshold;
#if (defined(CONFIG_HAS_EARLYSUSPEND) || defined(CONFIG_POWERSUSPEND) && !defined(DISABLE_POWER_MANAGEMENT)) || defined(USE_LCD_NOTIFIER)
		// ZZ: set grad_up_threshold sleep value
		if (interactive_profiles[i].grad_up_threshold_sleep < 100 && interactive_profiles[i].grad_up_threshold_sleep > 1)
		    dbs_tuners_ins.grad_up_threshold_sleep = interactive_profiles[i].grad_up_threshold_sleep;
#endif /* (defined(CONFIG_HAS_EARLYSUSPEND)... */
#ifdef ENABLE_HOTPLUGGING
		// ZZ: set hotplug_block_up_cycles value
		if (interactive_profiles[i].hotplug_block_up_cycles >= 0)
		    dbs_tuners_ins.hotplug_block_up_cycles = interactive_profiles[i].hotplug_block_up_cycles;

		// ZZ: set hotplug_block_down_cycles value
		if (interactive_profiles[i].hotplug_block_down_cycles >= 0)
		    dbs_tuners_ins.hotplug_block_down_cycles = interactive_profiles[i].hotplug_block_down_cycles;

		// ff: set hotplug_stagger_up value
		if (interactive_profiles[i].hotplug_stagger_up >= 0)
		    dbs_tuners_ins.hotplug_stagger_up = interactive_profiles[i].hotplug_stagger_up;

		// ff: set hotplug_stagger_down value
		if (interactive_profiles[i].hotplug_stagger_down >= 0)
		    dbs_tuners_ins.hotplug_stagger_down = interactive_profiles[i].hotplug_stagger_down;

		// ZZ: set hotplug_idle_threshold value
		if (interactive_profiles[i].hotplug_idle_threshold >= 0 && interactive_profiles[i].hotplug_idle_threshold < 100)
		    dbs_tuners_ins.hotplug_idle_threshold = interactive_profiles[i].hotplug_idle_threshold;

		// ZZ: set hotplug_idle_freq value
		if (interactive_profiles[i].hotplug_idle_freq == 0) {
		    dbs_tuners_ins.hotplug_idle_freq = interactive_profiles[i].hotplug_idle_freq;

		} else if (system_freq_table && interactive_profiles[i].hotplug_idle_freq <= system_freq_table[max_scaling_freq_hard].frequency) {
		    interactive_profiles[i].hotplug_idle_freq = check_frequency(interactive_profiles[i].hotplug_idle_freq);
		    for (t = 0; (system_freq_table[t].frequency != system_table_end); t++) {
			if (system_freq_table[t].frequency == interactive_profiles[i].hotplug_idle_freq) {
			    dbs_tuners_ins.hotplug_idle_freq = interactive_profiles[i].hotplug_idle_freq;
			}
		    }
		}
		...
		if (interactive_profiles[i].hotplug_engage_freq == 0) {
		    dbs_tuners_ins.hotplug_engage_freq = interactive_profiles[i].hotplug_engage_freq;

		} else if (system_freq_table && interactive_profiles[i].hotplug_engage_freq <= system_freq_table[max_scaling_freq_hard].frequency) {
		    interactive_profiles[i].hotplug_engage_freq = check_frequency(interactive_profiles[i].hotplug_engage_freq);
		    for (t = 0; (system_freq_table[t].frequency != system_table_end); t++) {
			if (system_freq_table[t].frequency == interactive_profiles[i].hotplug_engage_freq) {
			    dbs_tuners_ins.hotplug_engage_freq = interactive_profiles[i].hotplug_engage_freq;
			}
		    }
		}
		...
		if (interactive_profiles[i].hotplug_max_limit >= 0 && interactive_profiles[i].hotplug_max_limit < MAX_CORES)
		    dbs_tuners_ins.hotplug_max_limit = interactive_profiles[i].hotplug_max_limit;

		// ff: set hotplug_min_limit value
		if (interactive_profiles[i].hotplug_min_limit >= 0 && interactive_profiles[i].hotplug_min_limit < MAX_CORES)
		    dbs_tuners_ins.hotplug_min_limit = interactive_profiles[i].hotplug_min_limit;

		// ff: set hotplug_lock value
		if (interactive_profiles[i].hotplug_lock >= 0 && interactive_profiles[i].hotplug_lock < MAX_CORES)
		    dbs_tuners_ins.hotplug_lock = interactive_profiles[i].hotplug_lock;
#endif /* ENABLE_HOTPLUGGING */
#ifdef ENABLE_MUSIC_LIMITS
#ifdef ENABLE_HOTPLUGGING
		// ZZ: set music min cores value
		if (interactive_profiles[i].music_min_cores >= 0 && interactive_profiles[i].music_min_cores < MAX_CORES)
		    dbs_tuners_ins.music_min_cores = interactive_profiles[i].music_min_cores;
#endif /* ENABLE_HOTPLUGGING */

		// ZZ: set music min freq value
		if (interactive_profiles[i].music_min_freq == 0) {
		    dbs_tuners_ins.music_min_freq = interactive_profiles[i].music_min_freq;

		} else if (system_freq_table && interactive_profiles[i].music_min_freq <= system_freq_table[max_scaling_freq_hard].frequency) {
		    interactive_profiles[i].music_min_freq = check_frequency(interactive_profiles[i].music_min_freq);
		    for (t = 0; (system_freq_table[t].frequency != system_table_end); t++) {
			if (system_freq_table[t].frequency == interactive_profiles[i].music_min_freq) {
			    dbs_tuners_ins.music_min_freq = interactive_profiles[i].music_min_freq;
			}
		    }
		}
		...
		if (interactive_profiles[i].music_max_freq == 0) {
		    dbs_tuners_ins.music_max_freq = interactive_profiles[i].music_max_freq;

		} else if (system_freq_table && interactive_profiles[i].music_max_freq <= system_freq_table[max_scaling_freq_hard].frequency) {
		    interactive_profiles[i].music_max_freq = check_frequency(interactive_profiles[i].music_max_freq);
		    for (t = 0; (system_freq_table[t].frequency != system_table_end); t++) {
			if (system_freq_table[t].frequency == interactive_profiles[i].music_max_freq) {
			    dbs_tuners_ins.music_max_freq = interactive_profiles[i].music_max_freq;
			}
		    }
		}
		...
		if (interactive_profiles[i].scaling_responsiveness_freq == 0) {
		    dbs_tuners_ins.scaling_responsiveness_freq = interactive_profiles[i].scaling_responsiveness_freq;

		} else if (system_freq_table && interactive_profiles[i].scaling_responsiveness_freq <= system_freq_table[max_scaling_freq_hard].frequency) {
		    interactive_profiles[i].scaling_responsiveness_freq = check_frequency(interactive_profiles[i].scaling_responsiveness_freq);
		    for (t = 0; (system_freq_table[t].frequency != system_table_end); t++) {
			if (system_freq_table[t].frequency == interactive_profiles[i].scaling_responsiveness_freq) {
			    dbs_tuners_ins.scaling_responsiveness_freq = interactive_profiles[i].scaling_responsiveness_freq;
			}
		    }
		}
		...
		if (interactive_profiles[i].scaling_proportional > 3) {
		    dbs_tuners_ins.scaling_proportional = 3;
		} else if (interactive_profiles[i].scaling_proportional < 0) {
			dbs_tuners_ins.scaling_proportional = 1;
		} else {
			dbs_tuners_ins.scaling_proportional = interactive_profiles[i].scaling_proportional;
		}
#ifdef ENABLE_INPUTBOOSTER
		// ZZ: set inputboost cycles value
		if (interactive_profiles[i].inputboost_cycles >= 0 && interactive_profiles[i].inputboost_cycles <= 1000) {
			if (!interactive_profiles[i].inputboost_cycles && dbs_tuners_ins.inputboost_cycles != interactive_profiles[i].inputboost_cycles) {
			    // ff: input is 0, and it wasn't before.
			    // ff: so remove booster and unregister.
			    input_unregister_handler(&interactive_input_handler);
			} else if (interactive_profiles[i].inputboost_cycles && dbs_tuners_ins.inputboost_cycles == 0) {
		...
		dbs_tuners_ins.inputboost_cycles = interactive_profiles[i].inputboost_cycles;
		}

		// ZZ: set inputboost up threshold value
		if (interactive_profiles[i].inputboost_up_threshold <= 100)
		    dbs_tuners_ins.inputboost_up_threshold = interactive_profiles[i].inputboost_up_threshold;

		// ZZ: set inputboost punch cycles value
		if (interactive_profiles[i].inputboost_punch_cycles >= 0)
		    dbs_tuners_ins.inputboost_punch_cycles = interactive_profiles[i].inputboost_punch_cycles;

		// ZZ: set inputboost freq value
		if (interactive_profiles[i].inputboost_punch_freq == 0) {
		    dbs_tuners_ins.inputboost_punch_freq = interactive_profiles[i].inputboost_punch_freq;

		} else if (system_freq_table && interactive_profiles[i].inputboost_punch_freq <= system_freq_table[max_scaling_freq_hard].frequency) {
		    interactive_profiles[i].inputboost_punch_freq = check_frequency(interactive_profiles[i].inputboost_punch_freq);
		    for (t = 0; (system_freq_table[t].frequency != system_table_end); t++) {
			if (system_freq_table[t].frequency == interactive_profiles[i].inputboost_punch_freq) {
			    dbs_tuners_ins.inputboost_punch_freq = interactive_profiles[i].inputboost_punch_freq;
			}
		    }
		}
		...
		if (interactive_profiles[i].inputboost_punch_on_fingerdown >= 0)
		    dbs_tuners_ins.inputboost_punch_on_fingerdown = interactive_profiles[i].inputboost_punch_on_fingerdown;

		// ZZ: set inputboost punch punch on finger move value
		if (interactive_profiles[i].inputboost_punch_on_fingermove >= 0)
		    dbs_tuners_ins.inputboost_punch_on_fingermove = interactive_profiles[i].inputboost_punch_on_fingermove;

		// ZZ: set inputboost punch punch on epen move value
		if (interactive_profiles[i].inputboost_punch_on_epenmove >= 0)
		    dbs_tuners_ins.inputboost_punch_on_epenmove = interactive_profiles[i].inputboost_punch_on_epenmove;

		// ZZ: set inputboost up threshold value
		if (interactive_profiles[i].inputboost_typingbooster_up_threshold <= 100)
		    dbs_tuners_ins.inputboost_typingbooster_up_threshold = interactive_profiles[i].inputboost_typingbooster_up_threshold;
#ifdef ENABLE_HOTPLUGGING
		// ZZ: set inputboost cores value
		if (interactive_profiles[i].inputboost_typingbooster_cores >= 0 && interactive_profiles[i].inputboost_typingbooster_cores < MAX_CORES)
		    dbs_tuners_ins.inputboost_typingbooster_cores = interactive_profiles[i].inputboost_typingbooster_cores;
#endif /* ENABLE_HOTPLUGGING */
#endif /* ENABLE_INPUTBOOSTER */

		// ZZ: set scaling_responsiveness_up_threshold value
		if (interactive_profiles[i].scaling_responsiveness_up_threshold <= 100 && interactive_profiles[i].scaling_responsiveness_up_threshold >= 11)
		    dbs_tuners_ins.scaling_responsiveness_up_threshold = interactive_profiles[i].scaling_responsiveness_up_threshold;

		// ZZ: set ignore_nice_load value
		if (interactive_profiles[i].ignore_nice_load > 1)
		    interactive_profiles[i].ignore_nice_load = 1;

		dbs_tuners_ins.ignore_nice = interactive_profiles[i].ignore_nice_load;
		...
		if (interactive_profiles[i].sampling_down_factor <= MAX_SAMPLING_DOWN_FACTOR
		    && interactive_profiles[i].sampling_down_factor >= 1)
		    dbs_tuners_ins.sampling_down_factor = zz_sampling_down_factor = interactive_profiles[i].sampling_down_factor;
		...
		if (interactive_profiles[i].sampling_down_max_momentum <= MAX_SAMPLING_DOWN_FACTOR - dbs_tuners_ins.sampling_down_factor
		    && interactive_profiles[i].sampling_down_max_momentum >= 0) {
		    dbs_tuners_ins.sampling_down_max_mom = zz_sampling_down_max_mom = interactive_profiles[i].sampling_down_max_momentum;
		    orig_sampling_down_max_mom = dbs_tuners_ins.sampling_down_max_mom;
		}
		...
		if (interactive_profiles[i].sampling_down_momentum_sensitivity <= MAX_SAMPLING_DOWN_MOMENTUM_SENSITIVITY
		    && interactive_profiles[i].sampling_down_momentum_sensitivity >= 1) {
		    dbs_tuners_ins.sampling_down_mom_sens = interactive_profiles[i].sampling_down_momentum_sensitivity;
		...
		dbs_tuners_ins.sampling_rate = dbs_tuners_ins.sampling_rate_current
		= max(interactive_profiles[i].sampling_rate, min_sampling_rate);

		// ZZ: set sampling_rate_idle value
		if (interactive_profiles[i].sampling_rate_idle == 0) {
		    dbs_tuners_ins.sampling_rate_current = dbs_tuners_ins.sampling_rate
		    = dbs_tuners_ins.sampling_rate_idle;
		} else {
		    dbs_tuners_ins.sampling_rate_idle = max(interactive_profiles[i].sampling_rate_idle, min_sampling_rate);
		}

		// ZZ: set sampling_rate_idle_delay value
		if (interactive_profiles[i].sampling_rate_idle_delay >= 0) {
		    sampling_rate_step_up_delay = 0;
		    sampling_rate_step_down_delay = 0;
		    dbs_tuners_ins.sampling_rate_idle_delay = interactive_profiles[i].sampling_rate_idle_delay;
		}

		// ZZ: set sampling_rate_idle_threshold value
		if (interactive_profiles[i].sampling_rate_idle_threshold <= 100)
		    dbs_tuners_ins.sampling_rate_idle_threshold = interactive_profiles[i].sampling_rate_idle_threshold;
#if (defined(CONFIG_HAS_EARLYSUSPEND) || defined(CONFIG_POWERSUSPEND) && !defined(DISABLE_POWER_MANAGEMENT)) || defined(USE_LCD_NOTIFIER)
		// ZZ: set sampling_rate_sleep_multiplier value
		if (interactive_profiles[i].sampling_rate_sleep_multiplier <= MAX_SAMPLING_RATE_SLEEP_MULTIPLIER
		    && interactive_profiles[i].sampling_rate_sleep_multiplier >= 1)
		    dbs_tuners_ins.sampling_rate_sleep_multiplier = interactive_profiles[i].sampling_rate_sleep_multiplier;
#endif /* (defined(CONFIG_HAS_EARLYSUSPEND)... */
		// ZZ: set scaling_block_cycles value
		if (interactive_profiles[i].scaling_block_cycles >= 0) {
		    dbs_tuners_ins.scaling_block_cycles = interactive_profiles[i].scaling_block_cycles;
		    if (interactive_profiles[i].scaling_block_cycles == 0)
			scaling_block_cycles_count = 0;
		}
#ifdef CONFIG_EXYNOS4_EXPORT_TEMP
		// ZZ: set scaling_block_temp value
		if ((interactive_profiles[i].scaling_block_temp >= 30 && interactive_profiles[i].scaling_block_temp <= 80)
		    || interactive_profiles[i].scaling_block_temp == 0) {
		    dbs_tuners_ins.scaling_block_temp = interactive_profiles[i].scaling_block_temp;
		}
#endif /* CONFIG_EXYNOS4_EXPORT_TEMP */
		// ZZ: set scaling_block_freq value
		if (interactive_profiles[i].scaling_block_freq == 0) {
		    dbs_tuners_ins.scaling_block_freq = interactive_profiles[i].scaling_block_freq;

		} else if (system_freq_table && interactive_profiles[i].scaling_block_freq <= system_freq_table[max_scaling_freq_hard].frequency) {
		    interactive_profiles[i].scaling_block_freq = check_frequency(interactive_profiles[i].scaling_block_freq);
		    for (t = 0; (system_freq_table[t].frequency != system_table_end); t++) {
			if (system_freq_table[t].frequency == interactive_profiles[i].scaling_block_freq) {
			    dbs_tuners_ins.scaling_block_freq = interactive_profiles[i].scaling_block_freq;
			}
		    }
		}
		...
		if (interactive_profiles[i].scaling_block_threshold >= 0
		    && interactive_profiles[i].scaling_block_threshold <= 100)
		    dbs_tuners_ins.scaling_block_threshold = interactive_profiles[i].scaling_block_threshold;

		// ZZ: set scaling_block_force_down value
		if (interactive_profiles[i].scaling_block_force_down >= 0
		    && interactive_profiles[i].scaling_block_force_down != 1)
		    dbs_tuners_ins.scaling_block_force_down = interactive_profiles[i].scaling_block_force_down;

		// ZZ: set scaling_fastdown_freq value
		if (interactive_profiles[i].scaling_fastdown_freq == 0) {
		    dbs_tuners_ins.scaling_fastdown_freq = interactive_profiles[i].scaling_fastdown_freq;

		} else if (system_freq_table && interactive_profiles[i].scaling_fastdown_freq <= system_freq_table[max_scaling_freq_hard].frequency) {
		    interactive_profiles[i].scaling_fastdown_freq = check_frequency(interactive_profiles[i].scaling_fastdown_freq);
		    for (t = 0; (system_freq_table[t].frequency != system_table_end); t++) {
			if (system_freq_table[t].frequency == interactive_profiles[i].scaling_fastdown_freq) {
			    dbs_tuners_ins.scaling_fastdown_freq = interactive_profiles[i].scaling_fastdown_freq;
			}
		    }
		}
		...
		if (interactive_profiles[i].scaling_fastdown_up_threshold <= 100 && interactive_profiles[i].scaling_fastdown_up_threshold
		    > interactive_profiles[i].scaling_fastdown_down_threshold)
		    dbs_tuners_ins.scaling_fastdown_up_threshold = interactive_profiles[i].scaling_fastdown_up_threshold;

		// ZZ: set scaling_fastdown_down_threshold value
		if (interactive_profiles[i].scaling_fastdown_down_threshold < interactive_profiles[i].scaling_fastdown_up_threshold
		    && interactive_profiles[i].scaling_fastdown_down_threshold > 11)
		    dbs_tuners_ins.scaling_fastdown_down_threshold = interactive_profiles[i].scaling_fastdown_down_threshold;

		// ZZ: set smooth_up value
		if (interactive_profiles[i].smooth_up <= 100 && interactive_profiles[i].smooth_up >= 1)
		    dbs_tuners_ins.smooth_up = interactive_profiles[i].smooth_up;
#if (defined(CONFIG_HAS_EARLYSUSPEND) || defined(CONFIG_POWERSUSPEND) && !defined(DISABLE_POWER_MANAGEMENT)) || defined(USE_LCD_NOTIFIER)
		// ZZ: set smooth_up_sleep value
		if (interactive_profiles[i].smooth_up_sleep <= 100 && interactive_profiles[i].smooth_up_sleep >= 1)
		    dbs_tuners_ins.smooth_up_sleep = interactive_profiles[i].smooth_up_sleep;
#endif /* (defined(CONFIG_HAS_EARLYSUSPEND)... */
		// ZZ: set up_threshold value
		if (interactive_profiles[i].up_threshold <= 100 && interactive_profiles[i].up_threshold
		    >= interactive_profiles[i].down_threshold)
		    dbs_tuners_ins.up_threshold = interactive_profiles[i].up_threshold;
#ifdef ENABLE_HOTPLUGGING
		// ZZ: set up_threshold_hotplug1 value
		if (interactive_profiles[i].up_threshold_hotplug1 >= 0 && interactive_profiles[i].up_threshold_hotplug1 <= 100) {
		    dbs_tuners_ins.up_threshold_hotplug1 = interactive_profiles[i].up_threshold_hotplug1;
		    hotplug_thresholds[0][0] = interactive_profiles[i].up_threshold_hotplug1;
		}
#if (MAX_CORES == 4 || MAX_CORES == 6 || MAX_CORES == 8)
		// ZZ: set up_threshold_hotplug2 value
		if (interactive_profiles[i].up_threshold_hotplug2 >= 0 && interactive_profiles[i].up_threshold_hotplug2 <= 100) {
		    dbs_tuners_ins.up_threshold_hotplug2 = interactive_profiles[i].up_threshold_hotplug2;
		    hotplug_thresholds[0][1] = interactive_profiles[i].up_threshold_hotplug2;
		}

		// ZZ: set up_threshold_hotplug3 value
		if (interactive_profiles[i].up_threshold_hotplug3 >= 0 && interactive_profiles[i].up_threshold_hotplug3 <= 100) {
		    dbs_tuners_ins.up_threshold_hotplug3 = interactive_profiles[i].up_threshold_hotplug3;
		    hotplug_thresholds[0][2] = interactive_profiles[i].up_threshold_hotplug3;
		}
#endif /* (MAX_CORES == 4 || MAX_CORES == 6 || MAX_CORES == 8) */
#if (MAX_CORES == 6 || MAX_CORES == 8)
		// ZZ: set up_threshold_hotplug4 value
		if (interactive_profiles[i].up_threshold_hotplug4 >= 0 && interactive_profiles[i].up_threshold_hotplug4 <= 100) {
		    dbs_tuners_ins.up_threshold_hotplug4 = interactive_profiles[i].up_threshold_hotplug4;
		    hotplug_thresholds[0][3] = interactive_profiles[i].up_threshold_hotplug4;
		}

		// ZZ: set up_threshold_hotplug5 value
		if (interactive_profiles[i].up_threshold_hotplug5 >= 0 && interactive_profiles[i].up_threshold_hotplug5 <= 100) {
		    dbs_tuners_ins.up_threshold_hotplug5 = interactive_profiles[i].up_threshold_hotplug5;
		    hotplug_thresholds[0][4] = interactive_profiles[i].up_threshold_hotplug5;
		}
#endif /* (MAX_CORES == 6 || MAX_CORES == 8) */
#if (MAX_CORES == 8)
		// ZZ: set up_threshold_hotplug6 value
		if (interactive_profiles[i].up_threshold_hotplug6 >= 0 && interactive_profiles[i].up_threshold_hotplug6 <= 100) {
		    dbs_tuners_ins.up_threshold_hotplug6 = interactive_profiles[i].up_threshold_hotplug6;
		    hotplug_thresholds[0][5] = interactive_profiles[i].up_threshold_hotplug6;
		}

		// ZZ: set up_threshold_hotplug7 value
		if (interactive_profiles[i].up_threshold_hotplug7 >= 0 && interactive_profiles[i].up_threshold_hotplug7 <= 100) {
		    dbs_tuners_ins.up_threshold_hotplug7 = interactive_profiles[i].up_threshold_hotplug7;
		    hotplug_thresholds[0][6] = interactive_profiles[i].up_threshold_hotplug7;
		}
#endif /* (MAX_CORES == 8) */
		// ZZ: set up_threshold_hotplug_freq1 value
		if (interactive_profiles[i].up_threshold_hotplug_freq1 == 0) {
		    dbs_tuners_ins.up_threshold_hotplug_freq1 = interactive_profiles[i].up_threshold_hotplug_freq1;
		    hotplug_thresholds_freq[0][0] = interactive_profiles[i].up_threshold_hotplug_freq1;
		}

		if (system_freq_table && interactive_profiles[i].up_threshold_hotplug_freq1 <= system_freq_table[max_scaling_freq_hard].frequency) {
		    interactive_profiles[i].up_threshold_hotplug_freq1 = check_frequency(interactive_profiles[i].up_threshold_hotplug_freq1);
		    for (t = 0; (system_freq_table[t].frequency != system_table_end); t++) {
		        if (system_freq_table[t].frequency == interactive_profiles[i].up_threshold_hotplug_freq1) {
			    dbs_tuners_ins.up_threshold_hotplug_freq1 = interactive_profiles[i].up_threshold_hotplug_freq1;
			    hotplug_thresholds_freq[0][0] = interactive_profiles[i].up_threshold_hotplug_freq1;
		        }
		    }
		...
		if (interactive_profiles[i].up_threshold_hotplug_freq2 == 0) {
		    dbs_tuners_ins.up_threshold_hotplug_freq2 = interactive_profiles[i].up_threshold_hotplug_freq2;
		    hotplug_thresholds_freq[0][1] = interactive_profiles[i].up_threshold_hotplug_freq2;
		}

		if (system_freq_table && interactive_profiles[i].up_threshold_hotplug_freq2 <= system_freq_table[max_scaling_freq_hard].frequency) {
		    interactive_profiles[i].up_threshold_hotplug_freq2 = check_frequency(interactive_profiles[i].up_threshold_hotplug_freq2);
		    for (t = 0; (system_freq_table[t].frequency != system_table_end); t++) {
			if (system_freq_table[t].frequency == interactive_profiles[i].up_threshold_hotplug_freq2) {
			    dbs_tuners_ins.up_threshold_hotplug_freq2 = interactive_profiles[i].up_threshold_hotplug_freq2;
			    hotplug_thresholds_freq[0][1] = interactive_profiles[i].up_threshold_hotplug_freq2;
			}
		    }
		...
		if (interactive_profiles[i].up_threshold_hotplug_freq3 == 0) {
		    dbs_tuners_ins.up_threshold_hotplug_freq3 = interactive_profiles[i].up_threshold_hotplug_freq3;
		    hotplug_thresholds_freq[0][2] = interactive_profiles[i].up_threshold_hotplug_freq3;
		}

		if (system_freq_table && interactive_profiles[i].up_threshold_hotplug_freq3 <= system_freq_table[max_scaling_freq_hard].frequency) {
		    interactive_profiles[i].up_threshold_hotplug_freq3 = check_frequency(interactive_profiles[i].up_threshold_hotplug_freq3);
		    for (t = 0; (system_freq_table[t].frequency != system_table_end); t++) {
			if (system_freq_table[t].frequency == interactive_profiles[i].up_threshold_hotplug_freq3) {
			    dbs_tuners_ins.up_threshold_hotplug_freq3 = interactive_profiles[i].up_threshold_hotplug_freq3;
			    hotplug_thresholds_freq[0][2] = interactive_profiles[i].up_threshold_hotplug_freq3;
			}
		    }
		...
		if (interactive_profiles[i].up_threshold_hotplug_freq4 == 0) {
		    dbs_tuners_ins.up_threshold_hotplug_freq4 = interactive_profiles[i].up_threshold_hotplug_freq4;
		    hotplug_thresholds_freq[0][3] = interactive_profiles[i].up_threshold_hotplug_freq4;
		}

		if (system_freq_table && interactive_profiles[i].up_threshold_hotplug_freq4 <= system_freq_table[max_scaling_freq_hard].frequency) {
		    interactive_profiles[i].up_threshold_hotplug_freq4 = check_frequency(interactive_profiles[i].up_threshold_hotplug_freq4);
		    for (t = 0; (system_freq_table[t].frequency != system_table_end); t++) {
			if (system_freq_table[t].frequency == interactive_profiles[i].up_threshold_hotplug_freq4) {
			    dbs_tuners_ins.up_threshold_hotplug_freq4 = interactive_profiles[i].up_threshold_hotplug_freq4;
			    hotplug_thresholds_freq[0][3] = interactive_profiles[i].up_threshold_hotplug_freq4;
			}
		    }
		...
		if (interactive_profiles[i].up_threshold_hotplug_freq5 == 0) {
		    dbs_tuners_ins.up_threshold_hotplug_freq5 = interactive_profiles[i].up_threshold_hotplug_freq5;
		    hotplug_thresholds_freq[0][4] = interactive_profiles[i].up_threshold_hotplug_freq5;
		}

		if (system_freq_table && interactive_profiles[i].up_threshold_hotplug_freq5 <= system_freq_table[max_scaling_freq_hard].frequency) {
		    interactive_profiles[i].up_threshold_hotplug_freq5 = check_frequency(interactive_profiles[i].up_threshold_hotplug_freq5);
		    for (t = 0; (system_freq_table[t].frequency != system_table_end); t++) {
			if (system_freq_table[t].frequency == interactive_profiles[i].up_threshold_hotplug_freq5) {
			    dbs_tuners_ins.up_threshold_hotplug_freq5 = interactive_profiles[i].up_threshold_hotplug_freq5;
			    hotplug_thresholds_freq[0][4] = interactive_profiles[i].up_threshold_hotplug_freq5;
			}
		    }
		...
		if (interactive_profiles[i].up_threshold_hotplug_freq6 == 0) {
		    dbs_tuners_ins.up_threshold_hotplug_freq6 = interactive_profiles[i].up_threshold_hotplug_freq6;
		    hotplug_thresholds_freq[0][5] = interactive_profiles[i].up_threshold_hotplug_freq6;
		}

		if (system_freq_table && interactive_profiles[i].up_threshold_hotplug_freq6 <= system_freq_table[max_scaling_freq_hard].frequency) {
		    interactive_profiles[i].up_threshold_hotplug_freq6 = check_frequency(interactive_profiles[i].up_threshold_hotplug_freq6);
		    for (t = 0; (system_freq_table[t].frequency != system_table_end); t++) {
			if (system_freq_table[t].frequency == interactive_profiles[i].up_threshold_hotplug_freq6) {
			    dbs_tuners_ins.up_threshold_hotplug_freq6 = interactive_profiles[i].up_threshold_hotplug_freq6;
			    hotplug_thresholds_freq[0][5] = interactive_profiles[i].up_threshold_hotplug_freq6;
			}
		    }
		...
		if (interactive_profiles[i].up_threshold_hotplug_freq7 == 0) {
		    dbs_tuners_ins.up_threshold_hotplug_freq7 = interactive_profiles[i].up_threshold_hotplug_freq7;
		    hotplug_thresholds_freq[0][6] = interactive_profiles[i].up_threshold_hotplug_freq7;
		}

		if (system_freq_table && interactive_profiles[i].up_threshold_hotplug_freq7 <= system_freq_table[max_scaling_freq_hard].frequency) {
		    interactive_profiles[i].up_threshold_hotplug_freq7 = check_frequency(interactive_profiles[i].up_threshold_hotplug_freq7);
		    for (t = 0; (system_freq_table[t].frequency != system_table_end); t++) {
			if (system_freq_table[t].frequency == interactive_profiles[i].up_threshold_hotplug_freq7) {
			    dbs_tuners_ins.up_threshold_hotplug_freq7 = interactive_profiles[i].up_threshold_hotplug_freq7;
			    hotplug_thresholds_freq[0][6] = interactive_profiles[i].up_threshold_hotplug_freq7;
			}
		    }
		...
		if (interactive_profiles[i].up_threshold_sleep <= 100 && interactive_profiles[i].up_threshold_sleep
		    > dbs_tuners_ins.down_threshold_sleep)
		    dbs_tuners_ins.up_threshold_sleep = interactive_profiles[i].up_threshold_sleep;
#endif /* (defined(CONFIG_HAS_EARLYSUSPEND) */
#ifdef ENABLE_AUTO_ADJUST_FREQ
		// ZZ: set auto_adjust_freq_thresholds value
		if (interactive_profiles[i].auto_adjust_freq_thresholds > 1) {
		    interactive_profiles[i].auto_adjust_freq_thresholds = 1;
		    dbs_tuners_ins.auto_adjust_freq_thresholds = interactive_profiles[i].auto_adjust_freq_thresholds;
		} else {
		    dbs_tuners_ins.auto_adjust_freq_thresholds = interactive_profiles[i].auto_adjust_freq_thresholds;
		    pol_step = 0;
		}
		...
		strncpy(dbs_tuners_ins.profile, interactive_profiles[i].profile_name, sizeof(dbs_tuners_ins.profile));
...
// ZZ: tunable profile number -> for switching settings profiles, check interactive_profiles.h file for possible values
...
static ssize_t show_profile_list(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    for (i = 0; (interactive_profiles[i].profile_number != PROFILE_TABLE_END); i++) {
	c += sprintf(profiles+c, "profile: %d " "name: %s\n", interactive_profiles[i].profile_number,
	interactive_profiles[i].profile_name);
    }
...
#ifdef INTERACTIVE_DEBUG
// Yank: debug info
static ssize_t show_interactive_debug(struct device *dev, struct device_attribute *attr, char *buf)
{
...
static DEVICE_ATTR(interactive_debug, S_IRUGO , show_interactive_debug, NULL);
#endif /* INTERACTIVE_DEBUG */
...
#endif /* ENABLE_AUTO_ADJUST_FREQ */
#ifdef INTERACTIVE_DEBUG
	&dev_attr_interactive_debug.attr,
#endif /* INTERACTIVE_DEBUG */
	NULL
};
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	struct cpufreq_policy *policy;
#if defined(INTERACTIVE_DEBUG) && defined(ENABLE_INPUTBOOSTER)
	struct timeval time_now;
#endif /* INTERACTIVE_DEBUG */
...
#ifdef INTERACTIVE_DEBUG
			pr_info("[interactive/dbs_check_cpu] manual max-boost call! boosting to: %d mhz %d more times, cur: %d\n", policy->max, flg_ctr_cpuboost, policy->cur);
#endif /* INTERACTIVE_DEBUG */
		...
		if (flg_ctr_inputboost_punch > 0) {
			...
			if (!flg_ctr_inputboost_punch) {
#ifdef INTERACTIVE_DEBUG
				pr_info("[interactive/dbs_check_cpu] inputboost - punch expired\n");
#endif /* INTERACTIVE_DEBUG */
			} else {
				...
				if (policy->cur < dbs_tuners_ins.inputboost_punch_freq) {
					...
					pr_info("[interactive/dbs_check_cpu] time since touch: %d ms\n", time_since_touchbooster_lastrun);
					pr_info("[interactive/dbs_check_cpu] freq: %d too low, punching to %d immediately\n", policy->cur, dbs_tuners_ins.inputboost_punch_freq);
#endif /* INTERACTIVE_DEBUG */
	...
	for_each_cpu(j, policy->cpus) {
		 ...
		 * 'sticking' on max or relatively high frequency (caused by the very fast scaling behaving of interactive)
...
#ifdef INTERACTIVE_DEBUG
		pr_info("[interactive/dbs_check_cpu] 1 core\n");
#endif /* INTERACTIVE_DEBUG */
...
#ifdef INTERACTIVE_DEBUG
		pr_info("[interactive/dbs_check_cpu] 1 core - block up: %d, block down: %d\n", zz_hotplug_block_up_cycles, zz_hotplug_block_down_cycles);
#endif /* INTERACTIVE_DEBUG */
#if (MAX_CORES == 2 || MAX_CORES == 4 || MAX_CORES == 6 || MAX_CORES == 8)
	} else if (num_online_cpus == 2) {
#ifdef INTERACTIVE_DEBUG
		pr_info("[interactive/dbs_check_cpu] 2 cores\n");
#endif /* INTERACTIVE_DEBUG */
#if (MAX_CORES == 2)
...
#ifdef INTERACTIVE_DEBUG
		pr_info("[interactive/dbs_check_cpu] 2 cores - block up: %d, block down: %d\n", zz_hotplug_block_up_cycles, zz_hotplug_block_down_cycles);
#endif /* INTERACTIVE_DEBUG */
#endif /* (MAX_CORES == 2 || MAX_CORES == 4 || MAX_CORES == 6 || MAX_CORES == 8) */
#if (MAX_CORES == 4 || MAX_CORES == 6 || MAX_CORES == 8)
	} else if (num_online_cpus == 3) {
#ifdef INTERACTIVE_DEBUG
		pr_info("[interactive/dbs_check_cpu] 3 cores\n");
#endif /* INTERACTIVE_DEBUG */
...
#ifdef INTERACTIVE_DEBUG
		pr_info("[interactive/dbs_check_cpu] 3 cores - block up: %d, block down: %d\n", zz_hotplug_block_up_cycles, zz_hotplug_block_down_cycles);
#endif /* INTERACTIVE_DEBUG */
	} else if (num_online_cpus == 4) {
#ifdef INTERACTIVE_DEBUG
		pr_info("[interactive/dbs_check_cpu] 4 cores\n");
#endif /* INTERACTIVE_DEBUG */
#if (MAX_CORES == 4)
		    zz_hotplug_block_up_cycles = 0; // ff: if all cores are online, we can't go above that, so it's a moot setting
#else
...
#endif /* (MAX_CORES == 4) */
#ifdef INTERACTIVE_DEBUG
		pr_info("[interactive/dbs_check_cpu] 4 cores - block up: %d, block down: %d\n", zz_hotplug_block_up_cycles, zz_hotplug_block_down_cycles);
#endif /* INTERACTIVE_DEBUG */
#endif /* (MAX_CORES == 4 || MAX_CORES == 6 || MAX_CORES == 8) */
#if (MAX_CORES == 6 || MAX_CORES == 8)
	} else if (num_online_cpus == 5) {
#ifdef INTERACTIVE_DEBUG
		pr_info("[interactive/dbs_check_cpu] 5 cores\n");
#endif /* INTERACTIVE_DEBUG */
...
#ifdef INTERACTIVE_DEBUG
		pr_info("[interactive/dbs_check_cpu] 5 cores - block up: %d, block down: %d\n", zz_hotplug_block_up_cycles, zz_hotplug_block_down_cycles);
#endif /* INTERACTIVE_DEBUG */
	} else if (num_online_cpus == 6) {
#ifdef INTERACTIVE_DEBUG
		pr_info("[interactive/dbs_check_cpu] 6 cores\n");
#endif /* INTERACTIVE_DEBUG */
#if (MAX_CORES == 6)
		    zz_hotplug_block_up_cycles = 0; // ff: if all cores are online, we can't go above that, so it's a moot setting
#else
...
#endif /* (MAX_CORES == 6) */
#ifdef INTERACTIVE_DEBUG
		pr_info("[interactive/dbs_check_cpu] 6 cores - block up: %d, block down: %d\n", zz_hotplug_block_up_cycles, zz_hotplug_block_down_cycles);
#endif /* INTERACTIVE_DEBUG */
#endif /* (MAX_CORES == 6 || MAX_CORES == 8) */
#if (MAX_CORES == 8)
	} else if (num_online_cpus == 7) {
#ifdef INTERACTIVE_DEBUG
		pr_info("[interactive/dbs_check_cpu] 7 cores\n");
#endif /* INTERACTIVE_DEBUG */
...
#ifdef INTERACTIVE_DEBUG
		pr_info("[interactive/dbs_check_cpu] 7 cores - block up: %d, block down: %d\n", zz_hotplug_block_up_cycles, zz_hotplug_block_down_cycles);
#endif /* INTERACTIVE_DEBUG */
	} else if (num_online_cpus == 8) {
#ifdef INTERACTIVE_DEBUG
		pr_info("[interactive/dbs_check_cpu] 8 cores\n");
#endif /* INTERACTIVE_DEBUG */
		zz_hotplug_block_up_cycles = 0; // ff: if all cores are online, we can't go above that, so it's a moot setting
		zz_hotplug_block_down_cycles = dbs_tuners_ins.hotplug_block_down_cycles * dbs_tuners_ins.block_down_multiplier_hotplug7;
#ifdef INTERACTIVE_DEBUG
		pr_info("[interactive/dbs_check_cpu] 8 cores - block up: %d, block down: %d\n", zz_hotplug_block_up_cycles, zz_hotplug_block_down_cycles);
#endif /* INTERACTIVE_DEBUG */
#endif /* (MAX_CORES == 8) */
	}
...
#ifdef INTERACTIVE_DEBUG
		pr_info("[interactive/dbs_check_cpu] reset block counters\n");
#endif /* INTERACTIVE_DEBUG */
		hplg_up_block_cycles = 0;
		hplg_down_block_cycles = 0;
	}
	...
	if (flg_ctr_inputboost > 0 && !suspend_flag) {
		...
		if (flg_ctr_inputboost < 1) {
			pr_info("[interactive/dbs_check_cpu] inputboost event expired\n");
		}
#endif /* INTERACTIVE_DEBUG */
	}

#ifdef INTERACTIVE_DEBUG
	pr_info("[interactive/dbs_check_cpu] up_threshold: %d\n", scaling_up_threshold);
#endif /* INTERACTIVE_DEBUG */
#endif /* ENABLE_INPUTBOOSTER */
	...
	if ((max_load >= scaling_up_threshold || boost_freq) // ZZ: boost switch for early demand and scaling block switches added
...
#ifdef INTERACTIVE_DEBUG
			pr_info("[interactive/dbs_check_cpu] thermal throttling - cur freq: %d, max: %d, new freq: %d\n", policy->cur, policy->max, this_dbs_info->requested_freq);
#endif /* INTERACTIVE_DEBUG */
...
#ifdef INTERACTIVE_DEBUG
			pr_info("[interactive/dbs_check_cpu] inputboost - UP too low - repunched freq to %d, from %d\n", dbs_tuners_ins.inputboost_punch_freq, this_dbs_info->requested_freq);
#endif /* INTERACTIVE_DEBUG */
		...
		if (dbs_tuners_ins.scaling_up_block_cycles && !suspend_flag) {
			...
			if (policy->cur >= dbs_tuners_ins.scaling_up_block_freq) {
...
#ifdef INTERACTIVE_DEBUG
					pr_info("[interactive/dbs_check_cpu] scaling up BLOCKED #%d - cur freq: %d, target freq: %d\n", scaling_up_block_cycles_count, policy->cur, this_dbs_info->requested_freq);
#endif /* INTERACTIVE_DEBUG */
					return;
				} else {
					scaling_up_block_cycles_count = 0;
				...
				if (policy->cur < dbs_tuners_ins.scaling_up_block_freq) {
#ifdef INTERACTIVE_DEBUG
					pr_info("[interactive/dbs_check_cpu] scaling up RESET #%d - cur freq: %d, target freq: %d\n", scaling_up_block_cycles_count, policy->cur, this_dbs_info->requested_freq);
#endif /* INTERACTIVE_DEBUG */
					scaling_up_block_cycles_count = 0;
				}
			}
...
#endif /* ENABLE_MUSIC_LIMITS */
#ifdef INTERACTIVE_DEBUG
		pr_info("[interactive/dbs_check_cpu] scaling up SET - cur freq: %d, max: %d, new freq: %d\n", policy->cur, policy->max,  this_dbs_info->requested_freq);
#endif /* INTERACTIVE_DEBUG */
		__cpufreq_driver_target(policy, this_dbs_info->requested_freq,
			    CPUFREQ_RELATION_H);
...
#ifdef INTERACTIVE_DEBUG
			    pr_info("[interactive] skipped offline_work - %d / %d\n", hplg_down_block_cycles, zz_hotplug_block_down_cycles);
#endif /* INTERACTIVE_DEBUG */
...
#ifdef INTERACTIVE_DEBUG
		pr_info("[interactive/dbs_check_cpu] DOWN - down_threshold override (from: %d, to: %d)\n", scaling_down_threshold, down_threshold_override);
#endif /* INTERACTIVE_DEBUG */
		scaling_down_threshold = down_threshold_override;
	}
	...
	if (max_load < scaling_down_threshold || force_down_scaling) {				// ZZ: added force down switch
		...
		if (flg_ctr_inputboost_punch > 0 && this_dbs_info->requested_freq < dbs_tuners_ins.inputboost_punch_freq) {
			// ff: inputbooster punch is active and the the target freq needs to be at least that high
#ifdef INTERACTIVE_DEBUG
			pr_info("[interactive/dbs_check_cpu] inputboost - DOWN too low - repunched freq to %d, from %d\n", dbs_tuners_ins.inputboost_punch_freq, this_dbs_info->requested_freq);
#endif /* INTERACTIVE_DEBUG */
			this_dbs_info->requested_freq = dbs_tuners_ins.inputboost_punch_freq;
		}
#endif /* ENABLE_INPUTBOOSTER */
...
#ifdef INTERACTIVE_DEBUG
	pr_info("[interactive/zz_restartloop_work] restarting cycle\n");
#endif /* INTERACTIVE_DEBUG */
...
static void tmu_check_work(struct work_struct * work_tmu_check)
{
...
#ifdef INTERACTIVE_DEBUG
	pr_info("[interactive/thermal] sensor: %d, value: %ld\n", tsens_dev.sensor_num, temp);
#endif /* INTERACTIVE_DEBUG */
	tmu_temp_cpu = temp;
	...
	if (flg_ctr_tmu_overheating < 1) {
		...
		if (temp >= dbs_tuners_ins.scaling_trip_temp) {
			...
			ctr_tmu_neutral = 0;
#ifdef INTERACTIVE_DEBUG
			pr_info("[interactive/thermal] TRIPPED - was: %d, now: %d - step throttle: %d (%d mhz)\n",
		...
		if (tmu_temp_delta > 0) {
...
#ifdef INTERACTIVE_DEBUG
			pr_info("[interactive/thermal] RISING - was: %d, now: %d (delta: %d, eventdelta: %d) - step throttle: %d (%d mhz)\n",
			...
			if (ctr_tmu_falling > 1 && tmu_temp_cpu <= (dbs_tuners_ins.scaling_trip_temp + 2)) {
...
#ifdef INTERACTIVE_DEBUG
					pr_info("[interactive/thermal] fell too low - was: %d, now; %d\n", flg_ctr_tmu_overheating, max_scaling_freq_soft);
#endif /* INTERACTIVE_DEBUG */
					flg_ctr_tmu_overheating = max_scaling_freq_soft;
				}
				...
				tmu_throttle_steps = flg_ctr_tmu_overheating;
#ifdef INTERACTIVE_DEBUG
				pr_info("[interactive/thermal] FALLING - was: %d, now: %d (delta: %d, eventdelta: %d) - step throttle: %d (%d mhz)\n",
						tmu_temp_cpu_last, tmu_temp_cpu, tmu_temp_delta, tmu_temp_eventdelta, tmu_throttle_steps, system_freq_table[tmu_throttle_steps].frequency);

			} else {

				pr_info("[interactive/thermal] FALLING - IGNORING - was: %d, now: %d (delta: %d, eventdelta: %d) - step throttle: %d (%d mhz)\n",
						tmu_temp_cpu_last, tmu_temp_cpu, tmu_temp_delta, tmu_temp_eventdelta, tmu_throttle_steps, system_freq_table[tmu_throttle_steps].frequency);
#endif /* INTERACTIVE_DEBUG */
			}
			...
			if (ctr_tmu_neutral > 2 && tmu_temp_cpu >= (dbs_tuners_ins.scaling_trip_temp + 5)) {
				...
				tmu_throttle_steps = flg_ctr_tmu_overheating;
#ifdef INTERACTIVE_DEBUG
				pr_info("[interactive/thermal] STEADY - now: %d - step throttle: %d (%d mhz)\n", tmu_temp_cpu, tmu_throttle_steps, system_freq_table[tmu_throttle_steps].frequency);

			} else {
				pr_info("[interactive/thermal] STEADY - IGNORING - was: %d, now: %d (delta: %d, eventdelta: %d) - step throttle: %d (%d mhz)\n",
						tmu_temp_cpu_last, tmu_temp_cpu, tmu_temp_delta, tmu_temp_eventdelta, tmu_throttle_steps, system_freq_table[tmu_throttle_steps].frequency);
#endif /* INTERACTIVE_DEBUG */
			}
		}
...
static void __cpuinit hotplug_offline_work_fn(struct work_struct *work)
{
	...
	for_each_online_cpu(cpu) {
		if (likely(cpu_online(cpu) && (cpu)) && cpu != 0
			...
			pr_info("[interactive/hotplug_offline_work] turning off cpu: %d, load: %d / %d, min_limit: %d music_min: %d (saved: %d), min_touchbooster: %d, freq: %d / %d, mftl: %d\n",
			...
			pr_info("[interactive/hotplug_offline_work] turning off cpu: %d, load: %d / %d, min_limit: %d music_min: %d (saved: %d), min_touchbooster: %d, freq: %d, mftl: %d\n",
				cpu, cur_load, hotplug_thresholds[1][cpu-1], dbs_tuners_ins.hotplug_min_limit, dbs_tuners_ins.music_min_cores, dbs_tuners_ins.hotplug_min_limit_saved,
				dbs_tuners_ins.hotplug_min_limit_touchbooster, cur_freq, hotplug_thresholds_freq[1][cpu-1]);
#elif !defined(ENABLE_MUSIC_LIMITS) && defined(ENABLE_AUTO_ADJUST_FREQ)
			pr_info("[interactive/hotplug_offline_work] turning off cpu: %d, load: %d / %d, min_limit: %d (saved: %d), min_touchbooster: %d, freq: %d / %d, mftl: %d\n",
				cpu, cur_load, hotplug_thresholds[1][cpu-1], dbs_tuners_ins.hotplug_min_limit, dbs_tuners_ins.hotplug_min_limit_saved,
				dbs_tuners_ins.hotplug_min_limit_touchbooster, cur_freq, hotplug_thresholds_freq[1][cpu-1], hotplug_freq_threshold_out_of_range[1][cpu-1]);
#elif !defined(ENABLE_MUSIC_LIMITS) && !defined(ENABLE_AUTO_ADJUST_FREQ)
			pr_info("[interactive/hotplug_offline_work] turning off cpu: %d, load: %d / %d, min_limit: %d (saved: %d), min_touchbooster: %d, freq: %d, mftl: %d\n",
				cpu, cur_load, hotplug_thresholds[1][cpu-1], dbs_tuners_ins.hotplug_min_limit, dbs_tuners_ins.hotplug_min_limit_saved,
				dbs_tuners_ins.hotplug_min_limit_touchbooster, cur_freq, hotplug_thresholds_freq[1][cpu-1]);
...
#ifdef INTERACTIVE_DEBUG
				pr_info("[interactive/hotplug_offline_work] typing booster requested cpu%d on, trying next cpu...\n", cpu);
#endif /* INTERACTIVE_DEBUG */
				continue;
			}
#endif /* ENABLE_INPUTBOOSTER */
...
#ifdef INTERACTIVE_DEBUG
					pr_info("[interactive/hotplug_offline_work] higher cpu (%d) is still online, trying next cpu...\n", cpu + 1);
#endif /* INTERACTIVE_DEBUG */
					continue;
				}
#ifdef INTERACTIVE_DEBUG
				pr_info("[interactive/hotplug_offline_work] CPU %d OFF\n", cpu);
#endif /* INTERACTIVE_DEBUG */
				cpu_down(cpu);
	...
	if (flg_ctr_inputbooster_typingbooster > 0 && num_online_cpus() < dbs_tuners_ins.inputboost_typingbooster_cores) {
		for (i = 1; i < num_possible_cpus(); i++) {
			if (!cpu_online(i) && i < dbs_tuners_ins.inputboost_typingbooster_cores) {
				cpu_up(i);
#ifdef INTERACTIVE_DEBUG
				pr_info("[interactive/hotplug_online_work_fn/typingbooster] cpu%d forced online\n", i);
#endif /* INTERACTIVE_DEBUG */
			}
		}
	}
...
#ifdef INTERACTIVE_DEBUG
#if defined(ENABLE_MUSIC_LIMITS) && defined(ENABLE_AUTO_ADJUST_FREQ)
			pr_info("[interactive/hotplug_online_work] turning on cpu: %d, load: %d / %d, min_limit: %d music_min: %d (saved: %d), min_touchbooster: %d, freq: %d / %d, mftl: %d\n",
...
#elif defined(ENABLE_MUSIC_LIMITS) && !defined(ENABLE_AUTO_ADJUST_FREQ)
			pr_info("[interactive/hotplug_online_work] turning on cpu: %d, load: %d / %d, min_limit: %d music_min: %d (saved: %d), min_touchbooster: %d, freq: %d, mftl: %d\n",
				i, cur_load, hotplug_thresholds[0][i-1], dbs_tuners_ins.hotplug_min_limit, dbs_tuners_ins.music_min_cores, dbs_tuners_ins.hotplug_min_limit_saved,
				dbs_tuners_ins.hotplug_min_limit_touchbooster, cur_freq, hotplug_thresholds_freq[0][i-1]);
#elif !defined(ENABLE_MUSIC_LIMITS) && defined(ENABLE_AUTO_ADJUST_FREQ)
			pr_info("[interactive/hotplug_online_work] turning on cpu: %d, load: %d / %d, min_limit: %d (saved: %d), min_touchbooster: %d, freq: %d / %d, mftl: %d\n",
				i, cur_load, hotplug_thresholds[0][i-1], dbs_tuners_ins.hotplug_min_limit, dbs_tuners_ins.hotplug_min_limit_saved,
				dbs_tuners_ins.hotplug_min_limit_touchbooster, cur_freq, hotplug_thresholds_freq[0][i-1], hotplug_freq_threshold_out_of_range[0][i-1]);
#elif !defined(ENABLE_MUSIC_LIMITS) && !defined(ENABLE_AUTO_ADJUST_FREQ)
			pr_info("[interactive/hotplug_online_work] turning on cpu: %d, load: %d / %d, min_limit: %d (saved: %d), min_touchbooster: %d, freq: %d, mftl: %d\n",
				i, cur_load, hotplug_thresholds[0][i-1], dbs_tuners_ins.hotplug_min_limit, dbs_tuners_ins.hotplug_min_limit_saved,
				dbs_tuners_ins.hotplug_min_limit_touchbooster, cur_freq, hotplug_thresholds_freq[0][i-1]);
#endif /* defined(ENABLE_MUSIC_LIMITS)... */
#endif /* INTERACTIVE_DEBUG */
...
#ifdef INTERACTIVE_DEBUG
					pr_info("[interactive/hotplug_online_work] previous cpu (%d) was not online, aborting work\n", i - 1);
#endif /* INTERACTIVE_DEBUG */
					hotplug_up_in_progress = false;
					return;
				}
...
#ifdef INTERACTIVE_DEBUG
				pr_info("[interactive/hotplug_online_work] CPU %d ON\n", i);
#endif /* INTERACTIVE_DEBUG */
				cpu_up(i);
...
#endif /* defined(CONFIG_HAS_EARLYSUSPEND)... */
{
	...
	* sampling_rate_asleep;							// ZZ: set sampling rate for sleep
	...
	scaling_up_block_cycles_count = 0;
#ifdef INTERACTIVE_DEBUG
	pr_info("[interactive/lcd_notifier] Suspend function executed.\n");
#endif /* INTERACTIVE_DEBUG */
}
...
#ifdef INTERACTIVE_DEBUG
	pr_info("[interactive/lcd_notifier] Resume function executed.\n");
#endif /* INTERACTIVE_DEBUG */
}
	...
	case CPUFREQ_GOV_START:
		...
		if (dbs_enable == 1) {
			...
			if (!system_freq_table) {
			    printk(KERN_ERR "[interactive] Failed to get system freq table! falling back to proportional scaling!\n");
...
static int interactive_lcd_notifier_callback(struct notifier_block *this,
	...
	switch (event)
	{
		case LCD_EVENT_OFF_END:
			...
			if (!suspend_flag)
			    interactive_suspend();
#ifdef INTERACTIVE_DEBUG
			pr_info("[interactive/lcd_notifier] Screen switched off.\n");
#endif /* INTERACTIVE_DEBUG */
			break;
		...
		case LCD_EVENT_ON_START:
			...
			if (suspend_flag)
			    interactive_resume();
#ifdef INTERACTIVE_DEBUG
			pr_info("[interactive/lcd_notifier] Screen switched on.\n");
#endif /* INTERACTIVE_DEBUG */
			break;
...
static
#endif /* CONFIG_CPU_FREQ_DEFAULT_GOV_INTERACTIVE */
...
static int __init cpufreq_gov_dbs_init(void)						// ZZ: idle exit time handling
{
    ...
    dbs_wq = alloc_workqueue("interactive_dbs_wq", WQ_HIGHPRI, 0);
#ifdef ENABLE_WORK_RESTARTLOOP
    dbs_aux_wq = alloc_workqueue("interactive_dbs_aux_wq", WQ_HIGHPRI, 0);

    if (!dbs_aux_wq) {
	printk(KERN_ERR "[interactive] Failed to create interactive_dbs_aux_wq workqueue!\n");
	return -EFAULT;
    }
#endif /* ENABLE_WORK_RESTARTLOOP */
    if (!dbs_wq) {
	printk(KERN_ERR "[interactive] Failed to create interactive_dbs_wq workqueue!\n");
	return -EFAULT;
    }
	...
	interactive_lcd_notif.notifier_call = interactive_lcd_notifier_callback;
	if (lcd_register_client(&interactive_lcd_notif) != 0) {
...
#if (defined(USE_LCD_NOTIFIER) && !defined(CONFIG_POWERSUSPEND))
	lcd_unregister_client(&interactive_lcd_notif);
#endif /* (defined(USE_LCD_NOTIFIER)... */
}

MODULE_AUTHOR("Zane Zaminsky <cyxman@yahoo.com>");
MODULE_DESCRIPTION("'cpufreq_interactive' - A dynamic cpufreq governor based "
